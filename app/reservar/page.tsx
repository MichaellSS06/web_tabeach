...
import { MapPin, Calendar, Clock, Car, CreditCard, ShieldCheck } from 'lucide-react';
...
function FormularioReserva() {
  ...
  // Calculamos el precio directamente. Usamos useMemo para que el "random" 
  // no cambie cada vez que escribes una letra, solo cuando cambia el origen/destino.
  const precio = useMemo(() => {
    if (origen.length > 3 && destino.length > 3) {
      const tarifaBase = vehiculo.includes("Sprinter") ? 120 : 85;
      // Simulamos un cálculo basado en la longitud de los nombres (para que sea estable)
